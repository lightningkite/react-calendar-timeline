...
var TimelineElementsHeader = function (_Component) {
  ...
  _createClass(TimelineElementsHeader, [{
    ...
    value: function shouldComponentUpdate(nextProps, nextState) {
      var willUpdate = nextProps.canvasTimeStart != this.props.canvasTimeStart || nextProps.canvasTimeEnd != this.props.canvasTimeEnd || nextProps.width != this.props.width || nextProps.canvasWidth != this.props.canvasWidth || nextProps.subHeaderLabelFormats != this.props.subHeaderLabelFormats || nextProps.headerLabelFormats != this.props.headerLabelFormats || nextProps.groups != this.props.groups || nextState.dayHovered != this.state.dayHovered;
      ...
      (0, _calendar.iterateTimes)(canvasTimeStart, canvasTimeEnd, minUnit, timeSteps, function (time, nextTime) {
        ...
        bottomHeaderLabels.push(_react2.default.createElement(
          'div',
          {
            ...
            className: 'rct-label ' + (twoHeaders ? '' : 'rct-label-only') + ' ' + (firstOfType ? 'rct-first-of-type' : '') + ' ' + (minUnit !== 'month' ? 'rct-day-' + time.day() : '') + ' ' + (_this2.state.dayHovered === time.format('YYYY-MM-DD') && displayTooltip ? 'tooltip-hover' : ''),
